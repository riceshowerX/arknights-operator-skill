...
| **[C] 游戏数据 JSON** | PRTS Wiki 直接获取（`--source prts --name {角色名}`，自动从 PRTS API 获取并解析干员/敌人页面）或本地文件解析（`--source local --file {路径}`） |
| **[D] 同人创作** | 优秀的同人文章、图片说明 |
| **[E] 上传文件** | PDF / 图片 / 任意文本 |
| **[F] 直接粘贴** | 把文字复制进来 |
...
│   ├── game_data_parser.py        #   游戏资料解析（PRTS Wiki API 直接获取 / 本地文件 / slug 生成）
