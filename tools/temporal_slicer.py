...
def generate_temporal_rules(
    ...
    # 相邻时期比较
    baseline = slice_metrics.get(ordered[0], {})

    for i in range(1, len(ordered)):
        current = slice_metrics.get(ordered[i], {})
        diffs = compare_metrics(baseline, current)

        for diff in diffs:
            rule_text = f"{ordered[i]}时期相比{ordered[0]}时期：{diff['interpretation']}"
            rules.append({
                "rule": rule_text,
                "layer": 2,
                "metric": diff["metric"],
                "phases": [ordered[0], ordered[i]],
